...
fmt: goimports ## Run go fmt against code.
	${GOIMPORTS} -local github.com/Azure/hyperkv -w .
...
build: $(LOCALBIN) generate fmt vet ## Build manager binary.
	CGO_ENABLED=0 GOOS=linux GOARCH=${ARCH} go build -o bin/hyperkv ./cmd/
##@ Build Dependencies
...
$(GOYACC): $(LOCALBIN)
	test -s $(LOCALBIN)/goyacc || GOBIN=$(LOCALBIN) go install golang.org/x/tools/cmd/goyacc@latest